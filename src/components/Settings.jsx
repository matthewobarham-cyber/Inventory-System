...
export default function Settings({ isAdmin, accounts, userState, navConfig, auditEntries, importRuns, procurementRecords, vendors, approvalContacts, items, orders, borrowCategoryAccess = {}, accountStorage = 'Stored locally', onImport, onSaveVendor, onToggleVendor, onSaveApprovalContact, onToggleApprovalContact, onBorrowCategoryChange, onAccessChange, onToggle, onUpdateProfile, onCreateAccount, onResetPassword }) {
  ...
  return <div className="settings-workspace">
    ...
    <div className="settings-layout"><nav className="settings-nav" aria-label="Settings sections"><div><small>Configuration</small><strong>System settings</strong></div>{sections.map(([key, label, detail, icon]) => <button key={key} type="button" data-active={activeKey === key} onClick={() => setActive(key)}><span>{icon}</span><span><strong>{label}</strong><small>{detail}</small></span><i>›</i>{key === 'audit' && auditEntries.length > 0 && <b>{auditEntries.length}</b>}</button>)}</nav>
      <main className="settings-content"><header><span><small>{selected[1]}</small><strong>{selected[2]}</strong></span><span className="settings-local-badge">{activeKey === 'accounts' ? accountStorage : 'Stored locally'}</span></header><div className="settings-content-body">
