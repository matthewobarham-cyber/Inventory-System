...
export default function Loans({ items, canReturn, sender, emailContacts, onAddEmailContact, onEmailPrepared, onOpenItem, onCheckIn, onPreviewAgreement }) {
    ...
    <section className="loan-card-grid">
      {visible.map((item) => {
        ...
        return <article className="loan-card" data-state={state} data-stocktake-state={item.stocktakeState || undefined} key={item.id}>
          ...
          <footer>{item.loanAgreement ? <button type="button" className="btn-ghost" onClick={() => onPreviewAgreement(item.loanAgreement)}>View agreement</button> : <span>No agreement attached</span>}{canReturn && <><button type="button" className="loan-email-button" onClick={() => setEmailItem(item)}>Email borrower</button><button type="button" className="loan-return-button" onClick={() => onCheckIn(item.id)}>Check in asset</button></>}</footer>
        </article>;
      })}
